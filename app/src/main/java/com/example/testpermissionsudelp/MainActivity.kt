...
class MainActivity : AppCompatActivity() {
    ...
    val REQUEST_CODE = 1
    val permission =
        PermissionsApplications(this@MainActivity)
        ...
        when(requestCode){
            REQUEST_CODE->{
                if (grantResults.size>0){
                    ...
                    {
                        Toast.makeText(this@MainActivity,"Es obligatorio aceptar los permisos para utilizar esta aplicacion.", Toast.LENGTH_LONG).show()
                        finish()
                    }
                }else{
                    Toast.makeText(this@MainActivity,"Es obligatorio aceptar los permisos para utilizar esta aplicacion.", Toast.LENGTH_LONG).show()
